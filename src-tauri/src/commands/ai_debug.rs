            ...
            if let Some(first_choice) = choices.get(0) {
                if let Some(message) = first_choice.get("message") {
                    ...
                    if message.get("tool_calls").is_some() {
                        ...
                        logs.push(format!(
                            "The model responded with text instead of using tools."
                        ));
                    }
                }
    ...
    while let Some(event) = stream.next().await {
        ...
        match event {
            ...
            Ok(AgentEvent::ReasoningDelta(text)) => {
                logs.push(format!("[{}] ReasoningDelta: {}", event_count, text));
            }
            Ok(AgentEvent::UsageDelta(usage)) => {
                logs.push(format!(
                    "[{}] UsageDelta: prompt={:?} completion={:?} total={:?}",
