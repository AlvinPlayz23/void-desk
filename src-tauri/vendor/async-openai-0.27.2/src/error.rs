...
pub(crate) fn map_deserialization_error(e: serde_json::Error, bytes: &[u8]) -> OpenAIError {
    ...
    let raw_snippet = if raw.len() > 2000 {
        format!("{}\n\n[...truncated {} chars...]", &raw[..2000], raw.len() - 2000)
    } else {
        raw.to_string()
    };
    tracing::error!("failed deserialization of: {}", raw_snippet);
    OpenAIError::JSONDeserialize(serde_json::Error::custom(format!(
        "{} | raw: {}",
        e,
        raw_snippet
    )))
